...
function getNearDueDateCards() {
  var url = 'https://api.trello.com/1/boards/' + Config.trelloBoardId + '/cards?key=' + Config.trelloApiKey + '&token=' + Config.trelloToken;
...
function getMemberNames(card) {
  if (card.idMembers.length > 0) {
    var memberNames = card.idMembers.map(function(memberId) {
      var memberUrl = 'https://api.trello.com/1/members/' + memberId + '?key=' + Config.trelloApiKey + '&token=' + Config.trelloToken;
